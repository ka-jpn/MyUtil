...
public static class ExtensionMethods {
  ...
  ///[例1]nullじゃない場合にメソッド呼び出し:str=val?.Pipe(Math.Floor).Pipe(ToString);
  ///[例2]nullじゃない場合に操作:rcd=rcd?.Pipe(v=>v with{prop=""});</summary>
  ...
  ///[例1]nullじゃない場合にメソッド呼び出し:str?.Apply(strList.Add);
  ///[例2]nullじゃない場合に操作:objList?.Apply(v=>v.clear());</summary>
  public static T MyApply<T>(this T obj,Action<T> action,Predicate<T>? isInvoke = null) => obj.MyPipe(v => { action(v); return v; },isInvoke);
  ///<summary>リストの要素とそのインデックスをActionに渡すForEach</summary>
  public static void MyForEachWithIndex<T>(this IEnumerable<T> list,Action<T,int> action) => list.Index().ToList().ForEach(v => action(v.Item,v.Index));
  ...
  public static List<T> MyNonNull<T>(this IEnumerable<T?> list) => [.. list.OfType<T>()];
